...
**There is no per-tool wall time at all.** It is in the transcript nowhere, and `v1.0.3`
...
**v1.0.3 diagnoses only.** `--yes` is accepted, refused, and explained. The repair path is
held back because it can remove a registration that is *working*: it resolves
