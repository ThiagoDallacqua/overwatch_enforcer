...
**There is no per-tool wall time at all.** It is in the transcript nowhere, and `v1.0.0`
...
**v1.0.0 diagnoses only.** `--yes` is accepted, refused, and explained. The repair path is
